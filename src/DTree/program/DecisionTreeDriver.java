...
import DTree.datatype.NodeStatisticInfo;
import DTree.datatype.Rule;
import DTree.datatype.StatisticRecord;
...
public class DecisionTreeDriver {
    ...
    // 记录决策树模型，即所有确定的决策规则
    static Queue<Rule> model;
    static Configuration conf = new Configuration();

    public void DTmain(String datasetPath, String tempPath, String modelPath) throws Exception {
        model= new LinkedList<Rule>();
...
//    }
        // 存储数据属性元信息的文件的路径
        String attributesMetaInfoPath = tempPath + "/names";
        // 训练集数据文件所在路径
        String dataSetPath = datasetPath;
        // 当前统计结果所在文件夹的路径
        String statisticFilePath = tempPath + "/static";
        // 当前层节点队列文件的路径
        String nodeRuleQueueFilePath = tempPath + "/queue";
        // 模型文件存储路径
        String modelFilePath = modelPath;

        //生成names
        MakeNames.run(dataSetPath, attributesMetaInfoPath, tempPath);
        ...
        do {
            ...
            Map<Integer, NodeStatisticInfo> nodeStatisticInfos =
                    new LinkedHashMap<Integer, NodeStatisticInfo>();
            ...
            for (i = 0; i < Qlength; i++) {
                ...
                if (splitAid == 0) {
                    ...
                    newRule.conditions = new LinkedHashMap<>(rule.conditions);
